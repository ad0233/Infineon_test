...
static ble_data_recv_callback_t g_recv_callback = NULL;
static void *g_recv_context = NULL;
...
static uint8_t raw_scan_rsp_data[] = {
    /* Complete Local Name */
    0x0F, ESP_BLE_AD_TYPE_NAME_CMPL, 'L', 'u', 'n', 'a', 'w', 'a', 'k', 'e', '-', '1', '2', '3', '4', '5'   // Length 15, Data Type ESP_BLE_AD_TYPE_NAME_CMPL, Data (ESP_GATTS_DEMO)
};
#else
...
static void gap_event_handler(esp_gap_ble_cb_event_t event, esp_ble_gap_cb_param_t *param)
{
    ...
    case ESP_GAP_BLE_UPDATE_CONN_PARAMS_EVT:
         ESP_LOGD(GATTS_TAG, "Connection params update, status %d, conn_int %d, latency %d, timeout %d",
    ...
    case ESP_GAP_BLE_SET_PKT_LENGTH_COMPLETE_EVT:
        ESP_LOGD(GATTS_TAG, "Packet length update, status %d, rx %d, tx %d",
...
static void gatts_profile_a_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param) {
        ...
        if (!param->write.is_prep){
            ...
            if (gl_profile_tab[PROFILE_A_APP_ID].descr_handle == param->write.handle && param->write.len == 2){
                ...
                if (g_recv_callback != NULL) {
                    g_recv_callback(param->write.value, param->write.len, g_recv_context);
                }
            }
        }
    ...
    case ESP_GATTS_CONF_EVT:
        // ESP_LOGD(GATTS_TAG, "Confirm receive, status %d, attr_handle %d", param->conf.status, param->conf.handle);
        if (param->conf.status != ESP_GATT_OK){
            ESP_LOGE(GATTS_TAG, "Confirm failed, status %d, attr_handle %d", param->conf.status, param->conf.handle);
...
void my_ble_init(const char *device_name)
{
    ...
    // 初始化连接 ID
    for (int i = 0; i < PROFILE_NUM; i++) {
        gl_profile_tab[i].conn_id = 0xFFFF;
    }
