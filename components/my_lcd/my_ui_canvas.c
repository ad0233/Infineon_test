...
{
    ui_Canvas = lv_canvas_create(NULL);
    lv_obj_remove_flag(ui_Canvas, LV_OBJ_FLAG_SCROLLABLE);
    lv_obj_set_style_bg_color(ui_Canvas, lv_color_hex(0x000000), LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_bg_opa(ui_Canvas, 255, LV_PART_MAIN | LV_STATE_DEFAULT);
...
{
    if (ui_Canvas == NULL) {
        ESP_LOGE("my_ui_canvas", "ui_Canvas == NULL");
        return;
    }
    lvgl_port_lock(0);
    lv_disp_load_scr(ui_Canvas);
